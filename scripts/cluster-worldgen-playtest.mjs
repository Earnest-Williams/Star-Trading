...
function countReachableFrom(startId) {
    ...
    while (head < queue.length) {
        ...
        for (const gate of site.jumpGates || []) {
            if (!state.universe[gate.destinationSectorId] || visited.has(gate.destinationSectorId)) {
                continue;
            }
            visited.add(gate.destinationSectorId);
            queue.push(gate.destinationSectorId);
        }
    }
    return visited.size;
}
...
function runSeed(seed) {
    const clusterPreview = createSparseSitesFromClusterBlueprints(
        { archetypeKey: 'barred_spiral', occupiedSites: OCCUPIED_SITES, routeDensity: 1, chartedFraction: 0.6 },
        seededRng(seed)
    );
    ...
    const hints = Object.entries(clusterPreview.clusterHintsBySiteId || {});
    ...
    const economyProfileCount = Object.keys(state.economy?.profilesBySector || {}).length;
    const economyPressureExists = Object.values(state.economy?.pressureBySector || {}).some((pressureMap) => (
        pressureMap && Object.keys(pressureMap).length > 0
    ));
