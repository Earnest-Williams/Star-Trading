...
function formatFactionSummary(snapshot) {
    ...
    const factionLabel = faction ? `${faction.icon} ${faction.short}` : "Unaligned";
    return `${factionLabel}${status.status ? ` / ${status.status}` : ""}`;
}

function payloadFactionLabel(factionId) {
    const faction = FACTIONS[factionId];
    return faction ? `${faction.icon} ${faction.short}` : escapeHtml(factionId || "Unaligned");
}
